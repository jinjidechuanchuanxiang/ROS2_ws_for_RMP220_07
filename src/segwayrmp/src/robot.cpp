    ...
    std::shared_ptr<segway_msgs::srv::RosGetSwVersionCmd::Response> response)
{
    ...
    response->host_version = get_version(Host);
    response->chassis_version = get_version(Motor);
    response->route_version = get_version(route);
    RCLCPP_INFO(rclcpp::get_logger("SmartCar"), "ros_get_sw_version_cmd true");
    RCLCPP_INFO(rclcpp::get_logger("SmartCar"), "host_version:%d, chassis_version:%d, route_version:%d", 
        response->host_version, response->chassis_version, response->route_version);
}
    ...
    std::shared_ptr<segway_msgs::srv::RosGetVelMaxFeedbackCmd::Response> response)
{
    ...
    double forward_max = get_line_forward_max_vel_fb();
    double backward_max = get_line_backward_max_vel_fb();
    double angular_max = get_angular_max_vel_fb();
    response->forward_max_vel_fb = forward_max/CHASSIS_LINE_SPEED_SCALE;
    response->backward_max_vel_fb = backward_max/CHASSIS_LINE_SPEED_SCALE;
    response->angular_max_vel_fb = angular_max/CHASSIS_ANGULAR_SPEED_SCALE;
    ...
    std::shared_ptr<segway_msgs::srv::RosSetChassisEnableCmd::Response> response)
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("SmartCar"), "chassis_set_chassis_enable_result");
    response->chassis_set_chassis_enable_result = 1;
}
    ...
    std::shared_ptr<segway_msgs::srv::RosSetChassisPoweroffCmd::Response> response)
{
    ...
    response->chassis_set_poweroff_result = 1;
    RCLCPP_INFO(rclcpp::get_logger("SmartCar"), "chassis_set_poweroff_result");
}
    ...
    std::shared_ptr<segway_msgs::srv::RosSetLoadParamCmd::Response> response)
{
    ...
    set_chassis_load_state(value);
    response->chassis_set_load_param_result = 1;
    RCLCPP_INFO(rclcpp::get_logger("SmartCar"), "chassis_set_load_param_result");
}
    ...
    std::shared_ptr<segway_msgs::srv::RosSetRemovePushCmd::Response> response)
{
    ...
    set_remove_push_cmd();
    response->chassis_set_revove_push_result = 1;
    RCLCPP_INFO(rclcpp::get_logger("SmartCar"), "chassis_set_revove_push_result");
}
    ...
    std::shared_ptr<segway_msgs::srv::RosSetVelMaxCmd::Response> response)
{
    ...
    set_line_forward_max_vel(forward);
    set_line_backward_max_vel(backward);
    set_angular_max_vel(angular);
    ...
    RCLCPP_INFO(rclcpp::get_logger("SmartCar"), "set_line_forward_max_vel result, set_line_backward_max_vel result, set_angular_max_vel result");
}
...
void Chassis::iapCmdExecute(const std::shared_ptr<goalHandaleIapCmd> goal_handle)
{
    ...
    {
    case 1:
        RCLCPP_INFO(rclcpp::get_logger("SmartCar"), "iap chassis board");
        bins_directory = bins_directory + "/chassis.bin";
        bins_directory.copy(bin_dir, bins_directory.length(), 0);
        chassis_version.copy(ver, chassis_version.length(), 0);
        IapChassisBoard(bin_dir, ver);
        break;
    case 2:
        RCLCPP_INFO(rclcpp::get_logger("SmartCar"), "iap route board");
        bins_directory = bins_directory + "/route.bin";
        bins_directory.copy(bin_dir, bins_directory.length(), 0);
        route_version.copy(ver, route_version.length(), 0);
        IapRouteBoard(bin_dir, ver);
        break;
    default:
        RCLCPP_ERROR(rclcpp::get_logger("SmartCar"), 
        "iap_board value error, out of [1 2]", goal->iap_board);
        result->set__iap_result(4);
...
void Chassis::timer_1hz_callback(void)
{
    ...
    error_code_fb.chassis_error = get_err_state(Motor);
    error_code_fb.route_error = get_err_state(route);
...
void Chassis::speed_pub_callback(void)
{
    if (Speed_update == 1) {
        ...
        speed_fb.car_speed /= CHASSIS_LINE_SPEED_SCALE;
        speed_fb.turn_speed = SpeedData.turn_speed;
        speed_fb.turn_speed /= CHASSIS_ANGULAR_SPEED_SCALE;
        speed_fb.l_speed = SpeedData.l_speed;
        speed_fb.l_speed /= CHASSIS_LINE_SPEED_SCALE;
        speed_fb.r_speed = SpeedData.r_speed;
        speed_fb.r_speed /= CHASSIS_LINE_SPEED_SCALE;
...
{    
    if ((OdomPoseXy_update & OdomEulerXy_update & OdomEulerZ_update & OdomVelLineXy_update) == 1)  {
        ...
        odom_fb.twist.twist.linear.x = (double)SpeedData.car_speed / CHASSIS_LINE_SPEED_SCALE;
        odom_fb.twist.twist.linear.y = 0;
        odom_fb.twist.twist.angular.z = (double)SpeedData.turn_speed / CHASSIS_ANGULAR_SPEED_SCALE;;
