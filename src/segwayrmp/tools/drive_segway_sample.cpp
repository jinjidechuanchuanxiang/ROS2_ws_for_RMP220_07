...
{
    if (print_error != 0) {
        RCLCPP_INFO(rclcpp::get_logger("drive_segway_sample"), "host_error[%#x], chassis_error[%#x], \
        route_error[%#x]", 
        msg->host_error, msg->chassis_error, msg->route_error);
    }
}
    ...
    std::shared_ptr<segway_msgs::srv::ChassisSendEvent::Response> response)
{
    ...
    case OutEmergeStopEvent:
        ...
        break;
    case Locked_Rotor_Protect_Event:
